...
{
	gui_set_text_color(UI_COLOR_RED);	//设置字体颜色为蓝色
	gui_move_text_cursor(50, 65);		//设置字体位置
	gui_set_font(&MMI_medium_font);	//设置字体大小
	gui_print_text((UI_string_type)L"welcome TEST 3");
	gui_BLT_double_buffer(0, 0, UI_DEVICE_WIDTH, UI_DEVICE_HEIGHT);//设置分辨率
	kal_prompt_trace(MOD_MMS, "aplex mmi_my_mtk_func_exit\n");

}
//SCR_ID_MY_MTK_FUNC
void mmi_my_mtk_func(void)
{
	gui_set_text_color(UI_COLOR_DARK_GREY);	//设置字体颜色为蓝色
	gui_move_text_cursor(30, 45);		//设置字体位置
	gui_set_font(&MMI_medium_font);	//设置字体大小
	gui_print_text((UI_string_type)L"welcome TEST 2");
	gui_BLT_double_buffer(0, 0, UI_DEVICE_WIDTH, UI_DEVICE_HEIGHT);//设置分辨率
	kal_prompt_trace(MOD_MMS, "aplex mmi_my_mtk_func\n");
	//hj...
	SetKeyHandler(mmi_my_mtk_func_exit, KEY_LSK, KEY_EVENT_UP);
}

void mmi_my_mtk_func1(void)
{
	gui_set_text_color(UI_COLOR_WHITE);	//设置字体颜色为蓝色
	gui_move_text_cursor(10, 15);		//设置字体位置
	gui_set_font(&MMI_medium_font);	//设置字体大小
	gui_print_text((UI_string_type)L"welcome TEST 1");
	gui_BLT_double_buffer(0, 0, UI_DEVICE_WIDTH, UI_DEVICE_HEIGHT);//设置分辨率
	kal_prompt_trace(MOD_MMS, "aplex mmi_my_mtk_func1=%d\n");
...
void mmi_file_test_write()
{
	...
	//创建文件
	kal_wsprintf((unsigned short *)file, "%c:\\appData.txt", SRV_FMGR_PUBLIC_DRV);
	fd = FS_Open((WCHAR *)file, FS_READ_WRITE | FS_CREATE);
	if(fd<0)
	{
		kal_prompt_trace(MOD_MMS, "file open write error:%d\n", fd);
	}
...
void mmi_fresh_count()
{
	...
	//定时器
	StartTimer(TIMER_ID_MY_MTK_TIMER, 1000, mmi_fresh_count);

}

void mmi_my_mtk_test_timer(void)
{
...
/* under construction !*/
	...
	//mmi_my_mtk_test_timer();   //这个
	//kal_prompt_trace(MOD_MMI, "111111111\n");
	//mmi_my_mtk_func();
	//mtk_helloworld();
	//main_task_start();
#if 0
