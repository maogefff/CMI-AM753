...
static S32 my_two_spps_event_handler(U32 event, void* para)
{
	kal_prompt_trace(MOD_XDM, "-------my_spps_event_handler--------event=%d", event);
...
{	
	...
	kal_prompt_trace(MOD_XDM, "-----mmi_bt_spp_server_init-------");
	
	g_mmi_spp_cntx_p->handle = srv_spp_open();			//这里阻塞
	if (g_mmi_spp_cntx_p->handle == SRV_SPP_INVALID_HANDLE)
		return;

	kal_prompt_trace(MOD_XDM, "-----mmi_bt_spp_server_init11-------");
	...
	//测试代码	
#if 0
	evt_mask = MSG_ID_BT_SPP_ACTIVATE_CNF|
	...
	srv_spp_set_notify_hdlr(handle, evt_mask, my_two_spps_event_handler, NULL);
#endif
	//hej  不要认证
	//srv_spp_set_security_level(g_mmi_spp_cntx_p->handle, SRV_SPP_SEC_NAN);
	srv_spp_bind(g_mmi_spp_cntx_p->handle, SRV_BT_CM_SPP_PROFILE_UUID);
	kal_prompt_trace(MOD_XDM, "-----mmi_bt_spp_server_init222-------");
...
S32 mmi_bt_spp_event_handler(U32 evt, void* para)
{
	...
	kal_prompt_trace(MOD_XDM, "-----mmi_bt_spp_event_handler-------");
	kal_prompt_trace(MOD_XDM, "app_btspp_cb conn_id[%d]  ret = %d, evt[%d]",notify_para.para, notify_para.ret, evt);


	switch (evt)
	{
		case SRV_SPP_EVENT_BIND_FAIL:
			kal_prompt_trace(MOD_XDM, "SRV_SPP_EVENT_BIND_FAIL");
			mmi_bt_spp_bind_handler();
			break;
		case SRV_SPP_EVENT_AUTHORIZE:	// 1. 认证
			kal_prompt_trace(MOD_XDM, "SRV_SPP_EVENT_AUTHORIZE");
			mmi_bt_spp_authorize_ind(&notify_para);
			//srv_spp_accept(conn_id, btspp_buff, SRV_SPP_MIN_BUFF_SIZE / 2, SRV_SPP_MIN_BUFF_SIZE / 2);
			//srv_spp_accept(g_mmi_spp_cntx_p.conn_id, btspp_buff, SRV_SPP_MIN_BUFF_SIZE / 2, SRV_SPP_MIN_BUFF_SIZE / 2);
			/*
			srv_spp_accept(
		...
		case SRV_SPP_EVENT_CONNECT:
			kal_prompt_trace(MOD_XDM, "SRV_SPP_EVENT_CONNECT");
			mmi_bt_spp_connect_ind(&notify_para);
			break;
		case SRV_SPP_EVENT_READY_TO_WRITE:
			kal_prompt_trace(MOD_XDM, "SRV_SPP_EVENT_READY_TO_WRITE");
			break;
		case SRV_SPP_EVENT_READY_TO_READ:
			...
			//hej
			//mmi_bt_spp_receive_data();
			kal_prompt_trace(MOD_XDM, "SRV_SPP_EVENT_READY_TO_READ");
			break;
		case SRV_SPP_EVENT_DISCONNECT:
			kal_prompt_trace(MOD_XDM, "SRV_SPP_EVENT_DISCONNECT");
...
//hej 
/*
void mmi_send_spp_test()
{
	//send_spp_test(g_mmi_spp_cntx_p->conn_id);
}
*/
//hej
void test_send_spp_byaplex(void)
{
	send_spp_test_byaplex(g_mmi_spp_cntx_p->conn_id);
}

//hej

void send_bt_spp_byaplex(char* buff, int len)
{
	send_spp_byaplex(g_mmi_spp_cntx_p->conn_id, buff, len);
}
...
void mmi_bt_spp_authorize_ind(srv_spp_event_cntx* para)
{
	...
	//hej
	kal_prompt_trace(MOD_XDM, "mmi_bt_spp_authorize_ind");
	kal_prompt_trace(MOD_XDM, "dev  mac Addr:0x%02x:%02x:%02x:%02x:%02x:%02x", 
	...
	kal_prompt_trace(MOD_XDM, "start spp accept, g_mmi_spp_cntx_p->conn_id = %d", g_mmi_spp_cntx_p->conn_id);
...
void mmi_bt_spp_authorize_rsp(srv_bt_cm_bt_addr *bt_addr, U32 profile_id, U32 result)
{
	if (result)
	{
		kal_prompt_trace(MOD_XDM, "start spp accept");
		srv_spp_accept(
			g_mmi_spp_cntx_p->conn_id, 
			g_mmi_spp_cntx_p->buf, 
			sizeof(g_mmi_spp_cntx_p->buf)/2,
...
void mmi_bt_spp_test()
{
	...
	kal_prompt_trace(MOD_XDM, "mmi_bt_spp_test");
	dev_info = srv_bt_cm_get_paired_dev_info(0);
	bt_addr.lap = dev_info->bd_addr.lap;
	bt_addr.uap = dev_info->bd_addr.uap;
	bt_addr.nap = dev_info->bd_addr.nap;
	kal_prompt_trace(MOD_XDM, "dev[%s]:[0x%02x:%02x:%02x:%02x:%02x:%02x]", dev_info->name,
...
{
	srv_spp_event_cntx notify_para = *(srv_spp_event_cntx*)para;
	kal_prompt_trace(MOD_XDM, "------------mmi_bt_spp_client_hdlr--------------------");
	switch (evt)
	{
		case SRV_SPP_EVENT_BIND_FAIL:
...
S32 mmi_bt_sppc_notify_hdlr(U32 evt, void* para)
{
	...
	kal_prompt_trace(MOD_XDM, "------------mmi_bt_sppc_notify_hdlr--------------------");
		switch (evt)
		{
			case SRV_SPP_EVENT_CONNECT:
