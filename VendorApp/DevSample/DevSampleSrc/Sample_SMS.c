...
{
	kal_prompt_trace(MOD_XDM, "[message]mmi_sample_send_sms_callback");
	if (callback_data->result == MMI_TRUE)
	{
		/* send successfully */
		kal_prompt_trace(MOD_XDM, "[message]mmi_sample_send_sms_callback send successfully");
	}
    	else
	{
		kal_prompt_trace(MOD_XDM, "[message]mmi_sample_send_sms_callback send callback_data->cause=%d", callback_data->cause);
